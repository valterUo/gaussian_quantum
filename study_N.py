...
def run_N_study(tau=10, M=32, shots=4_194_304, seed=679, noise_std=0.01,
                quantum_M=8, quantum_noise_std=0.05,
...
def plot_N_study(records, out_path="figures/N_study.pdf"):
    ...
    # Classical HSGP-BQ with the quantum method's hyperparameters — the
    # exact curve the quantum estimate converges to as τ, shots → ∞.
    hsgpq_means = np.array([r["hsgp_q_mean"] for r in records])
    ...
    # Classical HSGP-BQ with the quantum hyperparameters (quantum target)
    ax.plot(n_vals, hsgpq_means, color=COLOR_HSGP_Q, lw=1.5,
            ls="-.", marker="D", markersize=4,
            label="HSGP-BQ (quantum hyperparams)")
