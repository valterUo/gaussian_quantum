...
def run_tau_study(N=16, M=16, shots=4_194_304, seed=679, noise_std=0.01,
...
def plot_tau_study(records, out_path="figures/tau_study.pdf"):
    ...
    # Classical HSGP-BQ with the quantum hyperparameters: the horizontal
    # line the quantum estimate must approach as τ grows.
    hsgpq_mean = float(np.mean([r["hsgp_q_mean"] for r in records]))
    ...
    ax.axhline(hsgpq_mean, color=COLOR_HSGP_Q, lw=1.2, ls="-.",
               label="HSGP-BQ (quantum hyperparams)", alpha=0.9)
